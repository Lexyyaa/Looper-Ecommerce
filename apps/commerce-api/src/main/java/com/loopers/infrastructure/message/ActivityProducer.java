...
public class ActivityProducer {
    ...
    public <T> void send(Envelope<T> envelope)  {
        ...
        ProducerRecord<Object, Object> record = new ProducerRecord<>(topic, envelope.actorId(), envelope);

        addHeaders(record, envelope.id(), eventType, envelope.at().toString(), envelope.actorId());

        try {

            kafkaSendRetryTemplate.execute(ctx -> { kafkaTemplate.send(record).get(); return null; });

        } catch (Exception e) {
            ProducerRecord<Object, Object> dead = new ProducerRecord<>(dlq, envelope.actorId(), envelope);
